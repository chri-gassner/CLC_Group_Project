#!/usr/bin/env python3
"""
demo_tabular.py (MediaPipe *Tasks* / "Open version" using PoseLandmarker .task)

Same behavior as your old demo_tabular:
- webcam/video
- downsample to FPS_TARGET
- sliding window WIN_FRAMES with STEP_FRAMES
- extract pose -> compute engineered per-frame features -> aggregate over window
- query trained TABULAR model (RF/XGB/SVM/KNN/MLP) and show skeleton + top1/top3
...
FONT_SCALE = 1.5     # <-- größer (z.B. 0.7 war vorher)
FONT_THICKNESS = 3   # <-- dicker
LINE_DY = 34         # <-- mehr Zeilenabstand
...
# =========================
# Pose connections robust (works even if mp.solutions is missing)
try:
    from mediapipe.python.solutions.pose import POSE_CONNECTIONS as MP_POSE_CONNECTIONS
except Exception:
    # fallback: hardcoded MediaPipe Pose connections (33 landmarks)
...
def overlay_text(img, lines, x=10, y=30, dy=26, font_scale=0.7, thickness=2):
...
def main():
    ...
    print(f"[INFO] Loaded model: {[model_name for _, model_name in models]}")
    ...
    try:
        while True:
            ...
            # pred_label, conf, topk, valid_frac = last_pred
            preds, valid_frac = last_pred

            lines = [
                f"valid_frac={valid_frac:.2f}  FPS_TARGET={FPS_TARGET} stride={stride} WIN={WIN_FRAMES} STEP={STEP_FRAMES}"]

            for mn, pl, cf, tk in preds[:2]:
                if np.isfinite(cf):
                    lines.append(f"{mn}: {pl}  conf={cf:.3f}")
                    lines.append("  Top3: " + " | ".join([f"{lbl}:{p:.2f}" for lbl, p in tk]))
                else:
                    lines.append(f"{mn}: {pl}")

            overlay_text(frame, lines, font_scale=FONT_SCALE, thickness=FONT_THICKNESS, dy=LINE_DY)

            cv2.imshow("demo_tabular (tasks) - Pose + Prediction", frame)
